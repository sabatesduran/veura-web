...
export interface Iphone17ProProps extends SVGProps<SVGSVGElement> {
  /** Image URL to show on the screen (e.g. "/screens/record.jpg") */
  src?: string;
  /** Optional explicit width/height for the SVG element (CSS sizing also works). */
  width?: number;
  height?: number;
}

/**
 * EldoraUI iPhone 17 Pro mockup (MIT licensed, copied into repo).
 * Source: https://github.com/karthikmudunuri/eldoraui
 */
export function Iphone17Pro({ width, height, src, ...props }: Iphone17ProProps) {
  // The original SVG paths are authored for a 200x400 coordinate system.
  // Keep the viewBox fixed so the geometry scales correctly when you change
  // CSS size or width/height.
  const baseWidth = 200;
